...
def asignar_turno(nombre,apellido,documento,medico_tratante,fecha_turno,hora_turno):
    conn= sql.connect("turnos.db")
    cursor=conn.cursor()
    instruccion=f"INSERT INTO turnos VALUES ('{nombre}','{apellido}',{documento},'{medico_tratante}','{fecha_turno}','{hora_turno}')"
    cursor.execute(instruccion)
    conn.commit()
    conn.close()
