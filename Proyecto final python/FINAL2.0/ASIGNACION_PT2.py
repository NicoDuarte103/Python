...
class aplication2():
    def __init__(self):
       ...
       # self.imagen_google = Image.open("imagenes_final2.0/imagenx.png").resize((20, 20))
       # self.imagen_google_tk = ImageTk.PhotoImage(self.imagen_google)
        ...
        self.label_reasig=tk.Label(self.frame, text="No disponible:" ,font=("Sitka Text Semibold", 30), width=500)
        self.label_reasig.pack(pady=9)

        self.label_mail=tk.Label(self.frame, text="No hay turnos disponibles\n para la especialidad\n solicitada.\n Oprima aceptar para\n dejar una solicitud." ,font=("Sitka Text Semibold", 20), width=500)
        self.label_mail.pack(pady=10)

        self.boton_gmail = ttk.Button(self.frame, width=50, text="ACEPTAR", image=self.imagen_google_tk, compound="left", command=gmail)
