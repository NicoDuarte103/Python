...
class aplication7():
    def __init__ (self):
        ...
        self.label_reasig=tk.Label(self.frame, text="Recuperar contraseña:" ,font=("Sitka Text Semibold", 30), width=500)
        self.label_reasig.pack(pady=9)

        self.label_mail=tk.Label(self.frame, text="Ingrese DNI:" ,font=("Sitka Text Semibold", 20), width=500)
        self.label_mail.pack(pady=12)

        self.entry_mail=ctk.CTkEntry(self.frame, width=300, height=30 ,fg_color="white", text_color="black")
        self.entry_mail.pack(pady=13)

        self.boton_confir = ttk.Button(self.frame, width=50, text="CONFIRMAR", image=self.imagen_contra_tk, compound="left", command=contra)
        self.boton_confir.pack(pady=14, padx=27)
