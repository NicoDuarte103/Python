...
class Aplicacion:
    ...
    def __init__(self):
        self.ventana1=tk.Tk()
        self.canvas1=tk.Canvas(self.ventana1, width=600, height=400, background="khaki2")
    ...
    def mover_mouse(self, evento):
        if self.presionado:
            self.canvas1.create_line(self.origenx,self.origeny,evento.x,evento.y, fill="magenta2")
            self.origenx=evento.x
            self.origeny=evento.y
